	...
	selectLink(post) {
		...
		apiFetch({
			...
			const fullpost = {
				...
				image: response.mlx_featured_image,
				author: author,
				url: response.link,
			}
		...
		return (
			<Fragment>
				...
				<div className="editor-url-input">
					<input autoFocus={autoFocus} type="text" aria-label={'URL'} required value={input} onChange={this.onChange} onInput={stopEventPropagation} placeholder={'Search for Page or Post name'} onKeyDown={this.onKeyDown} role="combobox" aria-expanded={showSuggestions} aria-autocomplete="list" aria-owns={`editor-url-input-suggestions-${instanceId}`} aria-activedescendant={selectedSuggestion !== null ? `editor-url-input-suggestion-${instanceId}-${selectedSuggestion}` : undefined} />
